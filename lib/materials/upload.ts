// ═══════════════════════════════════════════════
// CLIENTE DE UPLOAD — para usar en el frontend
// ═══════════════════════════════════════════════
...
export async function uploadMaterials(
  ...
  // ─── UPLOAD: subir directo a R2 en paralelo (máx 2 a la vez) ───
  ...
  for (let i = 0; i < files.length; i += 2) {
    ...
    await Promise.all(
      batch.map(async (file, bi) => {
        ...
        try {
          // Subir directo a R2 con XMLHttpRequest para tener progreso real
