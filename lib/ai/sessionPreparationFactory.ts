...
export function diagnoseEvaluationBlock(block:EvaluationPlanBlock,questions:PreparedEvaluationQuestion[],teaching:PreparedTeachingContent,evaluationPreference='mixed'):EvaluationBlockDiagnosis{
  ...
  for(const q of questions){const id=q.questionId||q.id||'unknown';const canonicalFormat=canonicalQuestionFormat(q.format);const blockIdOk=q.blockId===block.blockId;const stepIdsOk=Boolean(q.targetStepIds?.length)&&q.targetStepIds.every(step=>validSteps.has(step));const keyPointIdsOk=Boolean(q.targetKeyPointIds?.length)&&q.targetKeyPointIds.every(point=>validPoints.has(point));const factKeysOk=Boolean(q.targetFactKeys?.length)&&q.targetFactKeys.every(fact=>validFacts.has(fact));const promptOk=Boolean(q.prompt);const feedbackOk=true;const answerOk=canonicalFormat==='true_false'?typeof q.correctAnswer==='boolean':q.correctAnswer!==undefined&&q.correctAnswer!==null;const formatOk=Boolean(canonicalFormat)&&(evaluationPreference!=='quick_test'||quickFormats.has(canonicalFormat!));
    ...
    const structurallyValid=Boolean(id&&blockIdOk&&stepIdsOk&&keyPointIdsOk&&factKeysOk&&promptOk&&feedbackOk&&answerOk&&formatOk&&matchingOk&&cognitiveFormatFitOk);if(!structurallyValid){invalid.push(id);continue}const key=signature(q);if(seen.has(key)){duplicates.push(id);continue}seen.add(key);accepted.push(id);acceptedQuestions.push(q)}
...
export function diagnoseEvaluationCoverage(teaching: PreparedTeachingContent, blocks: PreparedEvaluationBlock[], evaluationPreference='mixed'): EvaluationCoverageDiagnosis {
  const diagnoses=blocks.map(block=>diagnoseEvaluationBlock(block,block.questions,teaching,evaluationPreference));const missingRequiredStepIds=uniq(diagnoses.flatMap(item=>item.missingRequiredStepIds));const missingCriticalKeyPoints=uniq(diagnoses.flatMap(item=>item.missingCriticalKeyPointIds));const missingImportantKeyPoints=uniq(diagnoses.flatMap(item=>item.missingImportantKeyPointIds));const invalidQuestionIds=uniq(diagnoses.flatMap(item=>item.invalidQuestionIds));const duplicateQuestionIds=uniq(diagnoses.flatMap(item=>item.duplicateQuestionIds));const affectedBlockIds=diagnoses.filter(item=>item.code==='PARTIAL_EVALUATION_COVERAGE').map(item=>item.blockId);return{code:affectedBlockIds.length?'PARTIAL_EVALUATION_COVERAGE':'EVALUATION_COMPLETE',missingRequiredStepIds,missingCriticalKeyPoints,missingImportantKeyPoints,invalidQuestionIds,duplicateQuestionIds,affectedBlockIds}
}
