...
export function diagnoseEvaluationCoverage(teaching: PreparedTeachingContent, blocks: PreparedEvaluationBlock[], evaluationPreference='mixed'): EvaluationCoverageDiagnosis {
  ...
  const diagnoses=blocks.map((block,index)=>diagnoseEvaluationBlock(block,block.questions,teaching,evaluationPreference,blocks.slice(0,index).flatMap(other=>other.questions)));const missingRequiredStepIds=uniq(diagnoses.flatMap(item=>item.missingRequiredStepIds));const missingCriticalKeyPoints=uniq(diagnoses.flatMap(item=>item.missingCriticalKeyPointIds));const missingImportantKeyPoints=uniq(diagnoses.flatMap(item=>item.missingImportantKeyPointIds));const missingFactKeys=uniq(diagnoses.flatMap(item=>item.missingFactKeys));const invalidQuestionIds=uniq(diagnoses.flatMap(item=>item.invalidQuestionIds));const duplicateQuestionIds=uniq(diagnoses.flatMap(item=>item.duplicateQuestionIds));const affectedBlockIds=diagnoses.filter(item=>item.code==='PARTIAL_EVALUATION_COVERAGE').map(item=>item.blockId);return{code:affectedBlockIds.length?'PARTIAL_EVALUATION_COVERAGE':'EVALUATION_COMPLETE',missingRequiredStepIds,missingCriticalKeyPoints,missingImportantKeyPoints,missingFactKeys,invalidQuestionIds,duplicateQuestionIds,affectedBlockIds}
}
