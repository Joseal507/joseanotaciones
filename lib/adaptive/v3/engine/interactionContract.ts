...
export const RAPID_FORMATS = new Set([
  ...
  'matching', 'ordering', 'numeric_short', 'practical_case', 'prediction',
])

const LONG_FORMATS = new Set([
  'open_response', 'teach_back', 'step_by_step_solver', 'practical_case',
  'explain_why', 'prediction', 'compare_contrast',
])
...
export function normalizeEvalPreference(value: unknown): EvalPreference {
  if (value === 'rapid' || value === 'quick' || value === 'quick_test') return 'quick_test'
...
export function validateInteractionContract(interaction: ContractInteraction, preference: EvalPreference, visibleContext = ''): string[] {
  ...
  if (preference === 'quick_test' && (!RAPID_FORMATS.has(type) || LONG_FORMATS.has(type))) errors.push(`FORMAT_DENIED_IN_RAPID: format ${type} denied in rapid`)
...
export function isPositiveBohrQuantumNumber(value: unknown): boolean {
