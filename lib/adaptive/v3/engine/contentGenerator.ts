...
import { prepareInteractionForDelivery } from './interactionContract'
import type {
  MicroConcept,
  MicroState,
  TeachingObjective,
...
export async function generateContent(request: GenerationRequest): Promise<GeneratedContent> {
  ...
  try {
    ...
    if (generated.interaction) {
      const prepared = prepareInteractionForDelivery(
        generated.interaction,
        request.evalPreference || 'mix_everything',
        {
          microId: micro.id,
          microName: micro.name,
          objective,
          sourceText: micro.sourceQuotes?.join(' ') || micro.fullDefinition || micro.shortDescription,
          usedFactKeys: avoidRepeating.filter(item => item.startsWith('FACT:')).map(item => item.slice(5)),
        },
        generated.blocks.map(block => JSON.stringify(block)).join(' '),
      )
...
- Ejemplos de repetición PROHIBIDA:
  ✗ Si preguntaste '¿Cuándo ocurrió el evento?' → NO preguntar '¿En qué año ocurrió el evento?'
...
function getInteractionSchema(format: string): string {
  const schemas: Record<string, string> = {
    ...
    "data": {
      "type": "fill_blank",
      "template": "El proceso comienza con ____",
      "correctAnswers": ["la observación"]
    }
  }`,
