...
test('introduction avanza, restaura y completa sin evaluación ni evidencia', async ({ page }) => {
  ...
  // Auditoría adversarial (Codex): una sesión sin evaluación nunca debe
  // mostrar un indicador de Dominio/objetivos — ni durante la sesión ni al
  // completarla — porque no hay evidencia real que respalde ningún número.
  await expect(page.getByText('Dominio')).toHaveCount(0)
  await expect(page.getByText(/\d+\/\d+ objetivos/)).toHaveCount(0)
  ...
  await expect(page.getByText('Familiarización completada')).toBeVisible()
  await expect(page.getByText('Dominio alcanzado')).toHaveCount(0)
  await expect(page.getByText(/\d+ de \d+ objetivos demostrados/)).toHaveCount(0)
  await page.reload()
  await expect(page.getByText('Sesión completada')).toBeVisible()
...
test('final_review restaura, completa y vuelve al mismo plan sin generación ni evaluación', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '🎉 Terminar' }).click()
  await expect(page.getByText('Sesión completada')).toBeVisible()
  await expect(page.getByText('Repaso completado')).toBeVisible()
  await expect(page.getByText('Dominio alcanzado')).toHaveCount(0)
