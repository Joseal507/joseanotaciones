...
test('showcase visual único: primitives reales, interacción, grading servidor y móvil',async({page})=>{
 ...
 for(const engine of ['graph_2d','geometry_canvas','structured_grid','spatial_vector','chemistry_2d','structure_graph','flow_state','timeline','source_image','code_execution','equation_expression'])await expect(page.locator(`[data-showcase-engine="${engine}"]`)).toBeVisible()
 ...
 const dcl=page.locator('[data-showcase-engine="spatial_vector"]');await expect(dcl.locator('[data-testid="spatial-vector-system"]')).toBeVisible();for(const label of ['Peso','Normal','Fuerza aplicada','Fricción'])await expect(dcl.getByText(label,{exact:true}).first()).toBeVisible();expect(await dcl.locator('line[marker-end="url(#arrow)"]').count()).toBeGreaterThanOrEqual(6);await expect(dcl.locator('[data-testid="vector-decomposition"]')).toContainText('Fx ≈ 25.98 N');await expect(dcl.locator('[data-testid="vector-decomposition"]')).toContainText('Fy = 15 N');await expect(dcl.locator('input')).toHaveCount(0)
 await page.setViewportSize({width:390,height:844});for(const engine of ['graph_2d','geometry_canvas','structured_grid','spatial_vector','structure_graph','flow_state','timeline','source_image','code_execution']){const box=await page.locator(`[data-showcase-engine="${engine}"]`).boundingBox();expect(box?.width).toBeLessThanOrEqual(390)}
