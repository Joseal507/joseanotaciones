...
// ─── Limpiar texto de flashcards: quita LaTeX innecesario, formatos rotos ───
function cleanFlashcardText(text: string): string {
  if (!text) return '';
  let t = String(text);
  // Quitar bloques $ ... $ y $ ... $ que envuelven texto que no es realmente matemática
  // Ejemplos malos: "$10,000.00$", "$100$ shares", "$10,000.00 divided into 100 shares with a value of $100.00 each.$"
  // 1. Quitar pares de $ que envuelven números con comas/puntos (precios)
  t = t.replace(/\$(\s*[\d,\.]+\s*)\$/g, '$1');
  // 2. Quitar $ sueltos que rodean texto plano (no math real)
  // Si el contenido entre $...$ no tiene operadores matemáticos reales, quitar los $
  t = t.replace(/\$([^$]+?)\$/g, (match, content) => {
    // Si tiene operadores matemáticos reales (\frac, \sqrt, ^, _, \sum, etc.) mantener
    if (/\\(frac|sqrt|sum|int|prod|lim|alpha|beta|gamma|theta|pi|infty|partial|nabla)|\^|_\{/.test(content)) {
      return match;
    }
    // Si no, devolver solo el contenido sin $
    return content;
  }); // <-- FIX: Cierre de la función de reemplazo
  // 3. Quitar bloques $ ... $ similares
  t = t.replace(/\$\$([^$]+?)\$\$/g, (match, content) => {
    if (/\\(frac|sqrt|sum|int|prod|lim)|\^\{|_\{/.test(content)) {
      return match;
    }
    return content; // <-- FIX: Asegurar un retorno para el caso 'else'
  }); // <-- FIX: Cierre de la función de reemplazo
  // 4. Quitar saltos de línea raros pegados a símbolos
  t = t.replace(/([\d,\.])\n([a-zA-Z])/g, '$1 $2');
  // 5. Limpiar espacios múltiples
  t = t.replace(/\s{2,}/g, ' ').trim();
  return t;
} // <-- FIX: Cierre de la función cleanFlashcardText
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        // Líneas horizontales sutiles (mismas en ambos lados)
        ...
        {/* Agujeros de carpeta */}
        <div style={{ // <-- FIX: Agregado el div de los agujeros de carpeta
          position: 'absolute',
        ...
        <div style={{ // <-- FIX: Agregado el div de la etiqueta
        ...
        {/* Contenido */}
        <div style={{ // <-- FIX: Agregado el div del contenido
          flex: 1,
          display: 'flex',
          alignItems: 'center',
          paddingLeft: 4,
        ...
        {/* Hint solo en grande */}
        {large && (
          <div style={{ // <-- FIX: Asegurado que este div esté correctamente abierto
...
function DashedButton({
  ...
  disabled?: boolean;
  fontSize?: number;
}) { // <-- FIX: Cierre correcto de la declaración de la función
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      style={{ // <-- FIX: Asegurado que este style esté correctamente abierto
        padding: '9px 18px',
        borderRadius: 12,
...
function CardMenu({
...
}: { // <-- FIX: Cierre correcto del bloque de tipos de props (se eliminó un ": {" extra)
  color: string;
  onStudySingle: () => void;
  onEdit: () => void;
  onShowSource: () => void;
  ...
  return ( // <-- FIX: Agregado el `return (` para el JSX
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ // <-- FIX: Asegurado que este div esté correctamente abierto
          ...
          display: 'flex', flexDirection: 'column', // <-- FIX: Añadido display para el menú
        }}>
          {[
            ...
            <button
              ...
              }} // <-- FIX: Cierre correcto del onClick
  ...
  ); // <-- FIX: Cierre del return de la función
}

// ─── PAGINACIÓN NUMÉRICA ESTILO SCREENSHOT ────────────────────
function NumberedPagination({
  total, current, onChange, color,
}: { // <-- FIX: Cierre correcto del bloque de tipos de props
  total: number;
  current: number;
  onChange: (i: number) => void;
  color: string; // <-- FIX: Añadido color a los props
}) { // <-- FIX: Cierre correcto de la declaración de la función
  // Para mazos grandes mostrar ventana inteligente
  const renderNumbers = () => {
    if (total <= 25) {
      // Mostrar todos
      return Array.from({ length: total }, (_, i) => i);
    }
    // Ventana: primeros 2, ... actuales-3 a actuales+3 ..., últimos 2
    ...
    if (current < 6) {
      ...
      showRange(0, 1); window.push('dots'); // <-- FIX: Se agregó la parte inicial de la ventana
        ...
        if (item === 'dots') {
          return (
            <span key={`dots-${idx}`} style={{
              ...
              padding: '0 6px', color: '#555', fontFamily: HAND, fontSize: 16,
            }}>···</span>
          );
        }
        const i = item;
...
function useFlashcardControls(cards: Flashcard[]) {
  ...
  useEffect(() => { // <-- FIX: useEffect para limpiar favoritos
  ...
  }, [cards, favorites]); // <-- FIX: Dependencias correctas para useEffect

  // ── Si no quedan favoritos y estábamos en modo favs_first/showOnlyFavs, resetear ──
  useEffect(() => { // <-- FIX: useEffect para resetear modos
    if (favorites.size === 0) {
  ...
  // ── Aplicar orden ──
  const sortedCards = useMemo(() => { // <-- FIX: Envolver en useMemo para optimización
    const arr = [...cards];
    switch (sortMode) {
      case 'material_first':
        return arr.sort((a, b) => {
      ...
      case 'material_last':
        return arr.sort((a, b) => { // <-- FIX: Añadido return y el cuerpo de la función
          const ma = a.sourceMaterialId || a.materialId || '';
          const mb = b.sourceMaterialId || b.materialId || '';
  ...
  }, [cards, sortMode, favorites]); // <-- FIX: Dependencias correctas para useMemo
...
function ControlsBar({ controls, color }: {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap', position: 'relative' }}>
      ...
      {/* Filtro favoritos */}
      {favorites.size > 0 && (
        <button // <-- FIX: Agregado el button para el filtro de favoritos
...
// ─── DECK VIEW (tab Flashcards: 1 grande con paginación) ─────
function DeckView({
  ...
  useEffect(() => { // <-- FIX: useEffect para resetear current
  ...
  useEffect(() => { // <-- FIX: useEffect para los listeners de teclado
  ...
  }, [visibleCards.length]); // <-- FIX: Dependencias actualizadas

  return (
    <div style={{ // <-- FIX: Agregado el div principal para DeckView
      ...
      {/* Badge IA verde */}
      <div style={{ // <-- FIX: Agregado el div para el badge de IA
        padding: '12px 18px', borderRadius: 12,
        background: 'rgba(74, 222, 128, 0.06)',
        border: '1.5px dashed rgba(74, 222, 128, 0.5)',
        color: '#86efac',
        fontSize: 16, fontFamily: BODY, fontWeight: 600,
      ...
      {/* Fila de botones de acción con bordes punteados */}
      <div style={{ // <-- FIX: Agregado el div para la fila de botones de acción
        ...
        <div style={{ fontSize: 16, fontFamily: HAND, color: '#999' }}>
      ...
      />
      {/* Contador grande con marco */}
      <div style={{ textAlign: 'center', maxWidth: 800, margin: '0 auto', width: '100%' }}> {/* FIX: Agregado div para el contador */}
        <span style={{
          display: 'inline-block', padding: '6px 18px',
          background: `${color}33`,
          border: `1.5px solid ${color}`,
          fontFamily: HAND, fontSize: 18, fontWeight: 700,
          color: color, borderRadius: 10, // <-- FIX: Añadido borderRadius
        }}>
          {current + 1} / {cards.length}
        </span>
      </div>
      {/* TARJETA GRANDE */}
      <div style={{ // <-- FIX: Agregado el div para la tarjeta grande
      ...
      <div style={{ // <-- FIX: Agregado el div para la navegación
        ...
        >← Anterior</button>
        <div style={{ fontFamily: HAND, fontSize: 14, color: '#666' }}>
      ...
      {/* Footer botones */}
      <div style={{ // <-- FIX: Agregado el div para los botones del footer
...
// ─── LISTA SCROLL (tab Material: una abajo de otra) ──────────
function ScrollList({
  ...
  const controls = useFlashcardControls(cards); // <-- FIX: Usar useFlashcardControls
  const { favorites, toggleFav, visibleCards } = controls; // <-- FIX: Desestructurar controls

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Header sticky con badge IA + estudiar */}
      <div style={{ // <-- FIX: Agregado el div para el header sticky
        padding: '14px 20px',
        borderBottom: '1px solid rgba(255,255,255,0.04)',
        background: '#0a0a0c',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', // <-- FIX: Añadido display
      }}>
        <div style={{ // <-- FIX: Agregado el div para el badge de IA
          ...
          marginBottom: 0, // <-- FIX: Eliminado el marginBottom innecesario
        }}>
      ...
      {/* Botones finales */}
      <div style={{ // <-- FIX: Agregado el div para los botones finales
...
function EditModal({ card, color, onSave, onClose }: {
  ...
  return (
    <div style={{ // <-- FIX: Agregado el div principal para el modal
      position: 'fixed', inset: 0,
      background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(12px)',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      zIndex: 2000, padding: 24,
    }}>
      <div style={{ // <-- FIX: Agregado el div para el contenido del modal
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <textarea
            ...
            style={{ // <-- FIX: Agregado el style para el textarea
...
// ─── STUDY: REPITE Y APRENDE (con IA + algoritmo) ──────────────
interface CardProgress {
  card: Flashcard; // <-- FIX: Añadido 'card' a CardProgress
  correctStreak: number;      // racha consecutiva de correctas
  wrongCount: number;
  mediumCount: number;
  correctCount: number;
  mastered: boolean;
  nextReviewIndex: number;    // posición en la cola
}

function StudyRepite({ cards, color, onClose, readOnly = false, contexto = '', order = 'bucle' }: {
  cards: Flashcard[]; // <-- FIX: Añadido cards a los props
  color: string; // <-- FIX: Añadido color a los props
  onClose: () => void; // <-- FIX: Añadido onClose a los props
  readOnly?: boolean;
  contexto?: string;
  order?: StudyOrder;
}) { // <-- FIX: Cierre correcto de la declaración de la función
  // ─── Algoritmo de repaso espaciado ───
  // - Si correcta 2 veces seguidas → aprendida
  // - Si incorrecta → vuelve en 2-3 cards
  // - Si medio_correcta → vuelve en 4-5 cards
  // - Si "no sé" → vuelve en 1-2 cards
  // - Si correcta (1ra vez) → vuelve en 6-8 cards
  ...
  useEffect(() => { // <-- FIX: useEffect para el foco del textarea
    if (!readOnly) textareaRef.current?.focus();
  }, [currentId, readOnly]);

  // ─── Obtener siguiente card según algoritmo ───
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      console.error("Error al evaluar con IA:", e); // <-- FIX: Añadido log de error
      setEvaluation({
  ...
  const continueNext = (resultLevel?: string) => {
    ...
    if (order === 'lineal') {
      ...
      if (isCorrect) {
        ...
        p.wrongCount++;
        p.correctStreak = 0; // <-- FIX: Resetear racha en "no sé"
        p.nextReviewIndex = position + 1 + Math.floor(Math.random() * 2);
      } else { // Incorrecta o muy_incorrecta
        p.wrongCount++;
        p.correctStreak = 0; // <-- FIX: Resetear racha en incorrecta
        p.nextReviewIndex = position + 2 + Math.floor(Math.random() * 2);
      } // <-- FIX: Cierre del bloque else (modo bucle)
    } // <-- FIX: Cierre del bloque else (modo lineal vs bucle)
  ...
  const handleDontKnow = async () => {
    // Simular una evaluación para mostrar la respuesta
    if (!current) return;
    setEvaluation({
      ...
      // No necesitas simular la llamada a la API si solo muestras la respuesta
    });
    setRevealed(true);
  }; // <-- FIX: Cierre de la función handleDontKnow
  ...
  if (done) {
    ...
    return (
      <div style={{ // <-- FIX: Agregado el div principal para la pantalla final
        ...
        <div style={{ fontSize: 16, color: '#888', fontFamily: HAND }}>
        ...
        <div style={{ display: 'flex', gap: 24, marginTop: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[
            ...
            <div key={s.label} style={{
              ...
              <div style={{ fontSize: 22, color: s.col }}>{s.icon}</div>
              <div style={{ fontSize: 42, fontWeight: 900, color: s.col, fontFamily: HAND, lineHeight: 1 }}>
                {s.val}
              </div>
              <div style={{ fontSize: 14, color: '#aaa', fontFamily: HAND, marginTop: 4 }}>
                {s.label}
              </div>
            </div>
          ))}
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#0a0a0c', display: 'flex', flexDirection: 'column', zIndex: 3000 }}>
        ...
        <div style={{ // <-- FIX: Agregado el div del cuerpo
          ...
          />
          <div style={{ fontSize: 16, color: '#666', fontFamily: HAND }}>
  ...
  const nivelColors: any = {
    ...
    dont_know: '#f87171', // <-- FIX: Añadido color para dont_know
  };
  const nivelLabels: any = {
    ...
    dont_know: '🤷 No sé', // <-- FIX: Añadido label para dont_know
  };

  // Stats totales actualizados en vivo
  ...
  const goBack = () => {
    ...
    setRevealed(false); // <-- FIX: Resetear revealed al ir atrás
    setEvaluation(null); // <-- FIX: Resetear evaluation al ir atrás
    setUserAnswer(''); // <-- FIX: Limpiar respuesta al ir atrás
  };

  const continueNextWithHistory = (resultLevel?: string) => {
    if (currentId) setHistory(h => [...h, currentId]); // <-- FIX: Solo añadir a historial si currentId existe
  ...
  }, [revealed, currentId, canGoBack, continueNextWithHistory, goBack]); // <-- FIX: Dependencias actualizadas
  ...
  const showAnswerOnly = () => {
    ...
    }); // <-- FIX: Cierre del objeto de evaluación
    setRevealed(true);
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0a0a0c', display: 'flex', flexDirection: 'column', zIndex: 3000 }}>
      ...
      <div style={{ // <-- FIX: Agregado el div del header
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: color, fontFamily: HAND }}>
      ...
      <div style={{ // <-- FIX: Agregado el div para las stats
        ...
        {[
          ...
          <div key={s.label} style={{
            ...
            <span style={{ fontSize: 13, color: '#aaa', fontFamily: HAND }}>{s.label}</span>
          </div>
        ))}
        {current.correctStreak > 0 && (
          <div style={{ // <-- FIX: Agregado el div para la racha
            display: 'flex', alignItems: 'center', gap: 8, // <-- FIX: Añadido display
            padding: '8px 14px', borderRadius: 10,
            background: `${color}22`,
            border: `1.5px dashed ${color}`,
          }}>
            <span style={{ fontSize: 16 }}>🔥</span>
            <span style={{ fontFamily: HAND, fontSize: 16, color: color, fontWeight: 700 }}>
        ...
        {!revealed ? (
          <div style={{ width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <DashedButton onClick={handleDontKnowWithHistory} color="#a78bfa" fontSize={15}>
                🤷 No sé
              </DashedButton>
              <DashedButton onClick={showAnswerOnly} color="#fbbf24" fontSize={15}>
                👁 Mostrar respuesta
              </DashedButton>
              <DashedButton
                onClick={submitAnswer}
                color={color} active fontSize={15}
          ...
          <div style={{ width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {evaluation && !evaluation._showOnly && (
              <div style={{ // <-- FIX: Agregado el div para la evaluación
                ...
                {/* Tu respuesta (si hubo) */}
                {userAnswer.trim() && (
                  <div>
                    <div style={{ // <-- FIX: Agregado el div para la etiqueta
                      fontSize: 12, color: '#888', fontFamily: HAND,
                ...
                {evaluation.respuestaCorrecta && (
                  <div style={{
                    ...
                    <div style={{ // <-- FIX: Agregado el div para la etiqueta
                      fontSize: 12, color: nivelColors[evaluation.nivel] || color,
                    ...
                    <div style={{
                      fontSize: 16, color: '#fff', fontFamily: BODY,
                ...
                {evaluation.consejo && (
                  <div style={{
                    ...
                    <strong style={{ fontFamily: BODY, fontSize: 15 }}>📌 Tip:</strong> {evaluation.consejo}
                  </div>
                )}
              </div>
            )}
            {evaluation?._showOnly && (
              <div style={{
                ...
                color: '#fbbf24', fontSize: 14, fontFamily: BODY,
                textAlign: 'center',
              }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
              {canGoBack && (
                <DashedButton onClick={goBack} color="#a78bfa" fontSize={16}>
                  ← Anterior
                </DashedButton>
              )}
              <button // <-- FIX: Agregado el button para Siguiente
...
// ─── STUDY: REPASO RÁPIDO (solo voltear y siguiente) ───────────
function StudyRapido({ cards, color, onClose, contexto = '', order = 'bucle' }: {
  ...
  const [shuffledCards] = useState<Flashcard[]>(() => { // <-- FIX: Declaración de shuffledCards
    if (order === 'bucle') {
      const arr = [...cards];
      for (let i = arr.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
  ...
  const [userAnswer, setUserAnswer] = useState(''); // <-- FIX: Añadido userAnswer
  const [evaluating, setEvaluating] = useState(false); // <-- FIX: Añadido evaluating
  const [evaluation, setEvaluation] = useState<any>(null); // <-- FIX: Añadido evaluation
  const [revealed, setRevealed] = useState(false); // <-- FIX: Añadido revealed
  const textareaRef = useRef<HTMLTextAreaElement>(null); // <-- FIX: Añadido textareaRef

  const card = shuffledCards[index];

  useEffect(() => { textareaRef.current?.focus(); }, [index]);

  const nivelColors: any = { // <-- FIX: Declaración de nivelColors
    INSANE: '#10b981', correcta: '#4ade80',
    medio_correcta: '#fbbf24', incorrecta: '#fb923c',
    muy_incorrecta: '#f87171', dont_know: '#f87171',
  };
  const nivelLabels: any = { // <-- FIX: Declaración de nivelLabels
    INSANE: '🏆 ¡PERFECTO!', correcta: '✓ Correcta',
    medio_correcta: '〜 Casi', incorrecta: '✗ Incorrecta',
    muy_incorrecta: '✗ Muy incorrecta', dont_know: '✗ No sabías',
  };

  const evaluate = async () => { // <-- FIX: Declaración de evaluate
  ...
  const showAnswer = async () => { // <-- FIX: Declaración de showAnswer
    if (!card) return;
    setEvaluation({
      nivel: 'dont_know', porcentaje: 0,
  ...
  if (index >= shuffledCards.length) {
    ...
    return (
      <div style={{ // <-- FIX: Agregado el div principal para la pantalla final
        ...
        <div style={{ fontSize: 16, color: '#888', fontFamily: HAND }}>Repasaste {shuffledCards.length} flashcards una vez</div>
        <div style={{ display: 'flex', gap: 24, marginTop: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
          {[
            ...
            <div key={s.label} style={{
              ...
              <div style={{ fontSize: 22, color: s.col }}>{s.icon}</div>
              <div style={{ fontSize: 42, fontWeight: 900, color: s.col, fontFamily: HAND, lineHeight: 1 }}>{s.val}</div>
              <div style={{ fontSize: 14, color: '#aaa', fontFamily: HAND, marginTop: 4 }}>{s.label}</div>
            </div>
          ))}
        </div>
        <button
          ...
          style={{
            ...
            fontFamily: HAND, fontSize: 20, fontWeight: 700, cursor: 'pointer',
          }} // <-- FIX: Cierre correcto del style
        >Terminar</button>
      </div>
    );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0a0a0c', display: 'flex', flexDirection: 'column', zIndex: 3000 }}>
      <div style={{ // <-- FIX: Agregado el div del header
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#888', fontSize: 16, cursor: 'pointer', fontFamily: HAND }}>← Salir</button>
        <div style={{ fontSize: 18, color: '#aaa', fontFamily: HAND }}>⚡ Repaso Rápido</div>
        <div style={{ fontSize: 16, fontWeight: 700, color, fontFamily: HAND }}>{index + 1}/{shuffledCards.length}</div>
      </div>
      <div style={{ height: 4, background: 'rgba(255,255,255,0.05)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '14px 20px 20px', gap: 16 }}>
        ...
        {!revealed ? (
          <div style={{ width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            />
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <DashedButton onClick={showAnswer} color="#fbbf24" fontSize={15}>👁 Mostrar respuesta</DashedButton>
              <DashedButton onClick={evaluate} color={color} active fontSize={15} disabled={evaluating || !userAnswer.trim()}>
                {evaluating ? '🤖 Evaluando...' : '✓ Enviar (Enter)'}
              </DashedButton>
            </div>
          </div>
          ...
          <div style={{ width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {evaluation && !evaluation._showOnly && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 12, borderBottom: '1px dashed ' + (nivelColors[evaluation.nivel] || color) + '33' }}>
                  ...
                  {evaluation.nivel !== 'dont_know' && (
                    <div style={{ fontFamily: HAND, fontSize: 32, fontWeight: 900, color: nivelColors[evaluation.nivel] || color, lineHeight: 1 }}>
                      {evaluation.porcentaje}%
                    </div>
                  )}
                </div>
                {userAnswer.trim() && (
                  <div>
                    <div style={{ fontSize: 12, color: '#888', fontFamily: HAND, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: 700 }}>✍️ Tu respuesta</div>
                ...
                {evaluation.analisis && (
                  <div>
                    <div style={{ fontSize: 12, color: '#888', fontFamily: HAND, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: 700 }}>🔍 Análisis</div>
                ...
                {evaluation.respuestaCorrecta && (
                  <div style={{ padding: '12px 14px', background: (nivelColors[evaluation.nivel] || color) + '15', border: '1.5px solid ' + (nivelColors[evaluation.nivel] || color) + '55', borderRadius: 10 }}>
                    <div style={{ fontSize: 12, color: nivelColors[evaluation.nivel] || color, fontFamily: HAND, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: 700 }}>✅ Respuesta correcta</div>
                    <div style={{ fontSize: 16, color: '#fff', fontFamily: BODY, lineHeight: 1.6, fontWeight: 500 }}>
                      <MathText text={cleanFlashcardText(evaluation.respuestaCorrecta)} />
                    </div>
                  </div>
                )}
                {evaluation.explicacion && (
                  <div>
                    <div style={{ fontSize: 12, color: '#888', fontFamily: HAND, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: 700 }}>💡 ¿Por qué?</div>
            ...
            {evaluation?._showOnly && (
              <div style={{ padding: 14, borderRadius: 12, background: 'rgba(251,191,36,0.08)', border: '1.5px dashed rgba(251,191,36,0.4)', color: '#fbbf24', fontSize: 14, fontFamily: BODY, textAlign: 'center' }}>
                👁 Respuesta revelada · Continúa cuando estés listo
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button onClick={next} style={{
                ...
                background: color + '22', color, fontFamily: HAND, fontSize: 18, fontWeight: 700, cursor: 'pointer',
              }}>
...
function StudySelector({ color, onSelect, onClose }: {
  ...
  return (
    <div style={{ // <-- FIX: Agregado el div principal para el selector
      ...
      <div style={{ // <-- FIX: Agregado el div para el contenido del selector
        ...
        {/* Toggle lineal / bucle */}
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, justifyContent: 'center' }}> {/* FIX: Añadido marginBottom */}
          {([
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 17, fontWeight: 700, color: opt.col, fontFamily: HAND }}>{opt.title}</div>
        ...
        <button
          ...
          style={{
            ...
            background: 'transparent', color: '#666', cursor: 'pointer', fontFamily: HAND, fontSize: 15,
          }} // <-- FIX: Cierre correcto del style
        >Cancelar</button>
      </div>
    </div>
...
function EmptyGenerate({ color, onGenerate, generating, numPages, selectedPages, materialesCount, activeMaterialIndex, totalSelectedPages }: {
  ...
  return (
    <div style={{ // <-- FIX: Agregado el div principal para el estado vacío
      flex: 1, display: 'flex', flexDirection: 'column',
      ...
      <div style={{ // <-- FIX: Agregado el div para la información de páginas
        padding: '10px 16px', borderRadius: 10,
        background: 'rgba(255,255,255,0.03)',
        border: '1.5px dashed rgba(255,255,255,0.15)',
        fontSize: 14, color: '#aaa', fontFamily: HAND,
      }}>
...
export default function FlashcardsPage({ materiales, seleccion, tema, materia, sessionId, onBack }: Props) {
  ...
  useEffect(() => { // <-- FIX: useEffect para actualizar la ref
    globalSelectedCursorRef.current = globalSelectedCursor;
  }, [globalSelectedCursor]);

  useEffect(() => { // <-- FIX: useEffect para resetear cursor si la secuencia cambia
    if (!selectionSequence.length && globalSelectedCursor !== 0) { // <-- FIX: Comprobación de longitud
      setGlobalSelectedCursor(0);
    } else if (selectionSequence.length > 0) { // <-- FIX: Solo ajustar si hay elementos
      setGlobalSelectedCursor(prev => {
        if (prev < 0 || prev >= selectionSequence.length) return 0;
        return prev;
      });
    }
  }, [selectionSequence.length]); // <-- FIX: Dependencia de longitud de la secuencia

  useEffect(() => { // <-- FIX: useEffect para sincronizar material activo
  ...
  const goToGlobalSelection = useCallback((nextIndex: number) => {
    ...
    // const entry = selectionSequence[safeIndex]; // No se usa, puede ser eliminado
  ...
  const goToPrev = useCallback(() => {
    goToGlobalSelection(globalSelectedCursorRef.current - 1);
  }, [goToGlobalSelection]); // <-- FIX: Cierre del useCallback
  ...
  useEffect(() => { // <-- FIX: useEffect para la carga del PDF
    let cancelled = false;
    let objectUrl = '';
    setPdfLoading(true);
    setPdfUrl(null);
    ...
    const loadUrl = async () => {
      ...
      if (matActual.archivo instanceof File) {
        ...
        return; // <-- FIX: Añadido return para evitar ejecutar el resto
      }
  ...
  // Cache del texto del material extraído (para pasarlo a evaluar)
  const [materialText, setMaterialText] = useState<string>('');

  // ── Cargar flashcards + materialText desde cache de sesión al montar ──
  useEffect(() => { // <-- FIX: useEffect para cargar cache
    if (!sessionId || cacheLoaded) return;
    try { // <-- FIX: Agregado try-catch
  ...
  useEffect(() => { // <-- FIX: useEffect para auto-guardar
    if (!sessionId || !cacheLoaded) return;
  ...
  }, [flashcards, sessionId, tema?.id, cacheLoaded, materialText]); // <-- FIX: Añadido materialText a dependencias

    // ─── Filtrar texto por páginas seleccionadas (ESTRICTO) ───
  const filterTextByPages = (fullText: string, pages: number[]): string => {
    if (!pages || pages.length === 0) return fullText;
    let pageTexts: string[] = [];
    // 1) Formato nuestro: marcador [Pagina N] + form feed
    ...
    // 2) Solo sin marcador
    // if (pageTexts.length <= 1 && fullText.includes('\f')) { // Esta condición está incompleta y no hace nada
    //   console.log(`📖 Split por \f solo: ${pageTexts.length} páginas`);
    // }
    // 3) Marcadores [Página N] como separadores
    ...
    if (pageTexts.length <= 1 && fullText.includes('[Page ')) {
      const parts = fullText
        .split(/(?=\[Page \d+\])/gi);
      if (parts.length > 1) { // <-- FIX: Agregado if para verificar si hay partes
        pageTexts = parts;
        console.log(`📖 Split por [Page N]: ${pageTexts.length} páginas`);
      }
    }
    // 5) Marcadores --- página N ---
    ...
    // ✅ Estricto: si no se puede separar, NO usar texto completo
    if (pageTexts.length <= 1) { // <-- FIX: Condición para cuando no se pudo separar
  ...
  const extractText = useCallback(async (): Promise<string> => {
    ...
    for (let i = 0; i < materiales.length; i++) { // <-- FIX: Bucle for
      ...
      // Si ya viene con texto pre-extraído (del enfoque), usarlo directo
      if ((sel as any)?.text) {
...
${fullText}`);
  ...
  const generate = useCallback(async () => {
    ...
    ); // <-- FIX: Cierre de setGeneratingStep
    ...
    try {
      ...
      if (!texto || texto.trim().length < 50) {
        ...
        if (sessionId) {
          try {
            const sessionsMap = JSON.parse(localStorage.getItem(`study_sessions_v1_${tema?.id || 'default'}`) || '{}'); // <-- FIX: Obtener el mapa de sesiones
            if (sessionsMap[sessionId]) {
              sessionsMap[sessionId].materialText = texto;
              localStorage.setItem(`study_sessions_v1_${tema?.id || 'default'}`, JSON.stringify(sessionsMap)); // <-- FIX: Guardar el mapa completo
      ...
      } else { // <-- FIX: Agregado else para el log
      ...
      const session = (await supabase.auth.getSession()).data.session; // <-- FIX: Obtener sesión aquí para usar en el map
      const allResponses = await Promise.all(
        materialBlocks.map(async (block, blockIdx) => {
          ...
          const data = await res.json(); // <-- FIX: Obtener data aquí
    ...
    } finally { // <-- FIX: Agregado finally
      clearInterval(progressInterval);
      clearInterval(stepInterval);
      setGenerating(false);
    }
  }, [extractText, matActual, hasAnySelection, totalSelectedPages, materialText, seleccion, sessionId, tema?.id]); // <-- FIX: Dependencias actualizadas
  ...
  const crearManualmente = () => {
    const nueva: Flashcard = {
      ...
      createdAt: Date.now(),
    }; // <-- FIX: Cierre del objeto
    setFlashcards(prev => [nueva, ...prev]);
    setEditingCard(nueva);
  };

  // Log movido fuera del render para evitar spam
  ...
  return (
    <div style={{ // <-- FIX: Agregado el div principal para FlashcardsPage
      position: 'fixed', inset: 0, background: '#0a0a0c',
      ...
      <div style={{ // <-- FIX: Agregado el div del header
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 24px', // <-- FIX: Añadido display
      }}>
        <button
          onClick={onBack}
          style={{
            ...
            fontFamily: HAND, fontSize: 15, cursor: 'pointer',
            transition: 'all 0.2s',
          }} // <-- FIX: Cierre correcto del style
          onMouseEnter={e => {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 14, color: '#555', fontFamily: HAND }}>·</span>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        {/* TABS */}
        <div style={{ // <-- FIX: Agregado el div para las tabs
          ...
          {materiales.length > 1 && (
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, paddingRight: 8 }}>
              <span style={{ fontSize: 14, color: '#666', fontFamily: HAND }}>Material:</span>
              <select
        ...
        {rightTab === 'material' && (
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            {/* PDF izquierda */}
            <div style={{ // <-- FIX: Agregado el div para el PDF izquierdo
              flex: '0 0 50%',
              ...
              {pdfLoading ? (
                <div style={{
                  ...
                  <div style={{ fontSize: 14, fontFamily: HAND }}>Cargando PDF...</div>
                </div>
              ) : pdfUrl ? (
                <PDFViewer
                ...
                <div style={{ // <-- FIX: Agregado el div para el mensaje de error
                  flex: 1, display: 'flex', flexDirection: 'column',
                  alignItems: 'center', justifyContent: 'center', gap: 12, color: '#555',
                }}>
                  <div style={{ fontSize: 36 }}>📄</div>
                  <div style={{ fontSize: 14, fontFamily: HAND }}>No se pudo cargar el material</div>
                </div>
              )}
            </div>
            {/* Derecha */}
        ...
        {/* TAB FLASHCARDS */}
        {rightTab === 'flashcards' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}> {/* FIX: Agregado div para flashcards */}
            {flashcards.length === 0 ? (
              <EmptyGenerate
                color={color} onGenerate={generate} generating={generating}
                numPages={numPages} selectedPages={selectedPages}
        ...
        {error && (
          <div style={{ // <-- FIX: Agregado el div para el error
            position: 'absolute', bottom: 20, left: '50%',
            transform: 'translateX(-50%)',
            padding: '10px 18px', borderRadius: 10,
            background: 'rgba(239,68,68,0.15)', border: '1.5px dashed rgba(239,68,68,0.5)',
            color: '#f87171', fontSize: 14, fontFamily: HAND, zIndex: 200,
          }}>{error}</div>
        )}
        {generating && (
          <div style={{ // <-- FIX: Agregado el div para el estado de generación
            ...
            <div style={{ textAlign: 'center', maxWidth: 360 }}>
              ...
              <div style={{ fontSize: 16, color: '#aaa', fontFamily: HAND, minHeight: 22 }}>
                {generatingStep}
              </div>
            </div>
            <div style={{
            ...
            <div style={{ fontSize: 14, color: '#666', fontFamily: HAND }}>
  ...
  color={color}
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Caveat:wght@400;700;800&display=swap');
