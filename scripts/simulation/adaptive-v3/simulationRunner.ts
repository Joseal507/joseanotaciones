...
export function measureStrategyChangesAfterRepeatedFailure(turns: SimulationTurn[]): {
  ...
  for (let index = 0; index < turns.length; index++) {
    const turn = turns[index]
    if (turn.format === 'none') {
      // Una intervención docente inicia un episodio pedagógico nuevo. Ignorarla
      // sin reiniciar la racha une fallos separados y crea oportunidades falsas.
      responseStreakByMicro.set(turn.microId, 0)
      continue
    }
