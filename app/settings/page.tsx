...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      // NO setear nombre aquí — se lee del perfil privado abajo
      // Solo como fallback temporal
      const googleName = data.user.user_metadata?.nombre || data.user.email?.split('@')[0] || '';
      ...
      try {
        ...
        if (up) {
          setNombre(up.nombre || googleName);
          setDescripcion(up.descripcion || '');
          setGenero(up.genero || '');
          setTipoEstudiante(up.tipo_estudiante || up.tipo_usuario || '');
          setEdad(String(up.edad || ''));
          setObjetivo(up.objetivo || '');
          if ((up.tipo_estudiante || up.tipo_usuario) === 'universitario') {
            setUniversidad(up.universidad || '');
            setCarrera(up.carrera || '');
          } else if ((up.tipo_estudiante || up.tipo_usuario) === 'escuela' || (up.tipo_estudiante || up.tipo_usuario) === 'estudiante') {
            setEscuela(up.escuela || up.universidad || '');
          }
        }
      } catch {}

      // El nombre ya se seteó arriba desde user-profile
      // Si user-profile no tenía nombre, usar Google como último fallback
      // (no necesitamos hacer nada porque setNombre(up.nombre || googleName) ya cubre ambos casos)

      // Cargar visible_leaderboard del perfil público
      try {
    ...
    try {
      {
        ...
        // Guardar en user-profile (fuente de verdad privada)
        // user-profile sincroniza automáticamente a perfil-publico
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', position: 'relative' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* ===== PERFIL ===== */}
          {seccion === 'perfil' && (
            <NotebookCard color="var(--gold)" emoji="👤" title={idioma === "en" ? "My Profile" : "Mi perfil"} rot={-0.4}>
              <Texto>{idioma === "en" ? "Your personal and academic information" : "Tu información personal y académica"}</Texto>
              ...
              {/* ── Datos privados (solo visibles para ti) ── */}
              <div style={{
                marginTop: 12,
                padding: '12px 16px',
                background: 'color-mix(in srgb, var(--blue) 8%, var(--bg-secondary))',
                ...
                <div style={{
                  ...
                  🔒 Datos privados (solo tú los ves)
                </div>
                ...
                <Field label="📅 Edad">
                  <Input
                    ...
                    max={120}
                    value={edad}
                    onChange={(e: any) => setEdad(e.target.value)}
                    placeholder="Tu edad"
                  />
                </Field>

                <Field label="🎯 Objetivo">
                  <Select value={objetivo} onChange={(e: any) => setObjetivo(e.target.value)}>
                    <option value="">Sin especificar</option>
                    <option value="mejorar_notas">Mejorar mis notas</option>
                    <option value="aprobar_examen">Aprobar un examen</option>
                    <option value="entender_materias">Entender mejor mis materias</option>
                    <option value="habito_estudio">Crear un hábito de estudio</option>
                    <option value="graduarme">Graduarme</option>
                  </Select>
                </Field>
              </div>
