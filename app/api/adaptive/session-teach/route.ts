...
// Última degradación de teaching: transforma exclusivamente texto ya grounded
// del blueprint en el mismo contrato canónico. No resume con heurísticas ni
// completa datos ausentes; por eso puede sustituir con seguridad una salida
// truncada del proveedor sin convertirlo en punto único de fallo.
export function buildGroundedTeachingFallback(body: TeachRequest): PreparedTeachingContent {
  const assigned=new Set(body.session.blockIds||[])
  const blocks=(body.blueprint.blocks||[]).filter((block:any)=>!assigned.size||assigned.has(String(block.id)))
  const topicBlocks=(body.blueprint.topics||[]).slice(0,5).map((topic:any,index:number)=>({id:`orientation_${index+1}`,label:String(topic.title||''),summary:String(topic.description||''),kind:'concept',importance:0}))
  const source=body.session.kind==='introduction'?topicBlocks:blocks
  const usable=source.filter((block:any)=>String(block.sourceQuote||block.summary||block.label||'').trim())
  if(!usable.length) throw new Error('GROUNDED_TEACHING_FALLBACK_UNAVAILABLE')
  const expanded=body.session.kind==='introduction'&&usable.length<3
    ? [...usable,...usable.slice(0,1),...usable.slice(0,1)].slice(0,3)
    : usable
  const steps=expanded.map((block:any,index:number)=>{
    const text=String(block.sourceQuote||block.summary||block.label).trim()
    const title=String(block.label||`Parte ${index+1}`).trim()
    const kind=String(block.kind||'concept')
    return {id:`step_${index+1}`,type:'concept' as const,title,content:text,keyPoints:[{id:`step_${index+1}:kp:1`,text}],microId:String(block.id||`grounded_${index+1}`),importance:block.importance>=100?'critical' as const:block.importance>=80?'important' as const:'supporting' as const,cognitiveTarget:kind==='formula'||kind==='example'?'application' as const:kind==='fact'?'recognition' as const:'comprehension' as const,relatedBlockIds:body.session.kind==='introduction'?[]:[String(block.id)],factKeys:[`${String(block.id)}:fact:1`],sourceReferences:[]}
  })
