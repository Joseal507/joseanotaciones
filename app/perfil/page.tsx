        ...
        <>
          <header style={{ background: 'var(--bg-card)', borderBottom: '3px solid var(--gold)', padding: '0 40px', height: '68px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                <h1 style={{ fontSize: '20px', fontWeight: 900, color: 'var(--text-primary)', margin: 0 }}>📊 Mi Perfil de Estudio</h1>
                <p style={{ color: 'var(--text-muted)', fontSize: '11px', margin: 0 }}>Tu progreso y estadísticas</p>
              </div>
            </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: isMobile ? '16px' : '40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '1px', background: 'var(--border-color)', borderRadius: '16px', overflow: 'hidden', marginBottom: '24px' }}>
          {[
            ...
            { label: 'Precisión', value: `${porcentajeGlobal}%`, color: 'var(--blue)', emoji: '🎯' },
          ].map((s, i) => (
              ...
              <div style={{ padding: '20px' }}>
                ...
                {topFalladas.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '24px 0' }}>
                    ...
                    <p style={{ color: 'var(--text-faint)', fontSize: '13px', margin: 0 }}>¡Aún no has fallado ninguna!</p>
                  </div>
                ) : (
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--gold-border)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <p style={{ fontSize: '11px', color: 'var(--text-faint)', margin: 0, textAlign: 'center' }}>
                  {xpTotal % 50}/50 XP → Nivel {nivel + 1}
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '14px' }}>
                  {[
                    { label: 'Total estudiadas', val: total, color: 'var(--gold)' },
                    { label: 'Precisión', val: `${porcentajeGlobal}%`, color: 'var(--blue)' },
                    { label: 'Materias', val: materiasOrdenadas.length, color: 'var(--pink)' },
                    { label: 'Nivel', val: nivel, color: 'var(--gold)' },
                  ].map((s, i) => (
            ...
            <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
                  {[
                    { emoji: '🌱', label: 'Primera flashcard', obtenido: total >= 1 },
                    { emoji: '⚡', label: '50 estudiadas', obtenido: total >= 50 },
                    { emoji: '🔥', label: '100 estudiadas', obtenido: total >= 100 },
                    { emoji: '💎', label: 'Nivel 5', obtenido: nivel >= 5 },
                    { emoji: '👑', label: 'Nivel 10', obtenido: nivel >= 10 },
                    { emoji: '🎯', label: '80% precisión', obtenido: porcentajeGlobal >= 80 && total > 10 },
                  ].map((logro, i) => (
                    ...
                    {materiasOrdenadas.filter(m => m.quizzes > 0).map((m, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg-secondary)', borderRadius: '8px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <span style={{ fontSize: '11px', color: 'var(--text-faint)' }}>{m.quizzes} quiz</span>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '24px', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => {
            if (!confirm('¿Limpiar racha y estadísticas?')) return;
