...
export default function TemaView({
  ...
  useEffect(() => {
    ...
    // Restaurar todo el estado de la sesión inmediatamente
    setSelectedIds(
      matIds
        .map((id: string) => {
    ...
    // Recuperar el studyMode GUARDADO en la sesión (free o adaptive)
    setStudyMode(lastSession.studyMode || 'free');
    ...
    console.log("⚡ Reabriendo StudyAL Process (instantáneo):", lastSession.id);
  ...
  if (openTeorico)
    return (
      <StudyALProcess
        ...
        masterySnapshot={masterySnapshot}
        initialMode={resumeProcessMode || studyMode || 'free'}
        onOpenStudyMap={() => {
    ...
    >
      ...
      {openTeorico === false &&
        ...
        (() => {
          ...
          const selectedMatIds = selectedDocs.map((d: any) =>
            getMaterialKey(d),
          );
          const matchingSession = activeSessions.find((s) => {
            if (s.materialIds.length !== selectedMatIds.length) return false;
            const sessionSet = new Set(s.materialIds.map(String));
            return selectedMatIds.every((id: string) =>
              sessionSet.has(String(id)),
            );
          });
                ...
                <button
                  onClick={() => {
                    if (isResumeMode && matchingSession) {
                      ...
                      // Recuperar el studyMode guardado
                      setStudyMode(matchingSession.studyMode || 'free');
                      // savedProcessMode ya está declarado arriba
                      setResumeProcessMode(savedProcessMode);
                      setOpenTeorico(true);
          ...
          onSelect={(id: string) => {
            if (id === "teorico" || id === "matematico" || id === "mixto") {
              ...
              if (matchingSession && matchingSession.selectedPages) {
                ...
                setSeleccionResult(rebuilt as any);
                setShowEnfoque(false);
                // Si la sesión ya tenía un studyMode guardado, usarlo directamente
                if (matchingSession.studyMode) {
                  setStudyMode(matchingSession.studyMode);
                  setResumeSessionId(matchingSession.id);
                  setOpenTeorico(true);
                } else {
      ...
      {showModeSelector && (
        <ModeSelector
          onSelectMode={(mode) => {
            ...
            // Si ya hay seleccionResult (venía de sesión existente), saltar a StudyALProcess
            if (Array.isArray(seleccionResult) && seleccionResult.length > 0) {
              setOpenTeorico(true);
            } else {
              // Si no, ir a seleccionar páginas
          ...
          onConfirm={(resultado) => {
            ...
            try {
              ...
              if (tema?.id && enfoqueElegido && matIds.length > 0) {
                ...
                setResumeSessionId(sess.id);
                refreshSessions();
                console.log("💾 Sesión creada al entrar al enfoque:", sess.id);
              }
            } catch (e) {
