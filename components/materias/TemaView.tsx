...
const buildSelectedMaterialsFingerprint = (
  ...
  const matIds = docs
    .map((d: any) => normalizeIdStrict(getMaterialKey(d)))
    .filter(Boolean)
    .sort();

  const matsKey = matIds.join("|");

  const pagesKey = Array.isArray(seleccionResult) && seleccionResult.length
    ? seleccionResult
        .map((s: any) => {
          const matId = normalizeIdStrict(s?.materialId);
          const pages = Array.isArray(s?.pages)
            ? [...new Set(s.pages.map(Number).filter((n: number) => Number.isFinite(n) && n > 0))].sort((a: number, b: number) => a - b)
            : [];
          return `${matId}:${pages.join(",")}`;
        })
        .filter(Boolean)
        .sort()
        .join("|")
  ...
  return `${matsKey}__PAGES__${pagesKey}`;
};

const buildSessionMaterialsFingerprint = (session: any) => {
  const matIds = (session?.materialIds || [])
    .map((id: any) => normalizeIdStrict(id))
    .filter(Boolean)
    .sort();

  const matsKey = matIds.join("|");

  const selectedPages = session?.selectedPages || {};
  const pagesKey = Object.entries(selectedPages)
    .map(([matId, pages]) => {
      const cleanPages = Array.isArray(pages)
        ? [...new Set((pages as any[]).map(Number).filter((n) => Number.isFinite(n) && n > 0))].sort((a, b) => a - b)
        : [];
      return `${normalizeIdStrict(matId)}:${cleanPages.join(",")}`;
    })
    .filter(Boolean)
    .sort()
    .join("|");
...
export default function TemaView({
  ...
  const [resumeProcessMode, setResumeProcessMode] = useState<'free' | 'adaptive' | null>(null);
  // Ref para el modo elegido — nunca se pisa por guards o re-renders
  const chosenModeRef = useRef<'free' | 'adaptive' | null>(null);
  ...
  useEffect(() => {
    ...
    const currentFP = buildSelectedMaterialsFingerprint(
      selectedDocs,
      seleccionResult,
    );
    const resumeFP = buildSessionMaterialsFingerprint(resumeSession);

    if (currentFP !== resumeFP) {
      setResumeSessionId(null);
      setResumeProcessMode("free" as any);
      // Solo limpiar studyMode si el usuario no eligió un modo explícitamente
  ...
  const reconstructedMasteryState = useMemo(() => {
    ...
    return masteryState;
  }, [resumeSessionId, openTeorico, tema?.id, masteryState]);
    ...
    return (
      <StudyALProcess
        ...
        onClose={() => {
          ...
          // NO borrar studyMode/resumeProcessMode al cerrar
        ...
        onOpenRepasar={() => {
          ...
          try {
            ...
            if (tema?.id && matIds.length > 0) {
              const _repasarMode: 'free' | 'adaptive' = (() => {
                if (studyMode) return studyMode;
                if (resumeProcessMode) return resumeProcessMode;
                const existing = activeSessions.find(s => matIds.every((id: string) => s.materialIds.includes(id)));
                if (existing) return (existing.processMode || existing.studyMode || 'free') as 'free' | 'adaptive';
      ...
      {openTeorico === false &&
        ...
        (() => {
          ...
          const selectedMatIds = selectedDocs
            .map((d: any) => normalizeId(getMaterialKey(d)))
            .filter(Boolean)
            .sort();

          const selectedKey = selectedMatIds.join('|');

          const currentSelectedPagesKey = (() => {
            if (!Array.isArray(seleccionResult) || !seleccionResult.length) return '';
            const normalized = seleccionResult
              .map((s: any) => {
                const matId = normalizeId(s?.materialId);
                const pages = Array.isArray(s?.pages)
                  ? [...new Set(s.pages.map(Number).filter((n: number) => Number.isFinite(n) && n > 0))].sort((a: number, b: number) => a - b)
                  : [];
                return `${matId}:${pages.join(',')}`;
          ...
          // Blindaje máximo por fingerprint exacto de selección
          const currentSelectionFingerprint = buildSelectedMaterialsFingerprint(
            selectedDocs,
            seleccionResult,
          );
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                >
                  ...
                  >
                    {isResumeMode
                      ? matchingSession?.enfoque || "teorico"
          ...
          onConfirm={(resultado) => {
            ...
            try {
              ...
              if (tema?.id && matIds.length > 0) {
                ...
                savedSessionId = sess.id;
                setResumeSessionId(sess.id);
                setResumeProcessMode(_seleccionMode);
                setStudyMode(_seleccionMode);
                refreshSessions();
                console.log("💾 Sesión SeleccionPaginas:", sess.id, "| processMode:", _seleccionMode);
              }
            } catch (e) {
