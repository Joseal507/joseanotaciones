...
export default function Buscador({ onClose }: Props) {
  ...
  const PAGINAS = [
    ...
    { titulo: 'ChapBot', subtitulo: idioma === 'en' ? 'Chat with AI' : 'Chat con AI', emoji: '🤖', href: '/chat', keywords: ['chapbot', 'chat', 'ai', 'bot'] },
      ...
      try {
        ...
        getQuizzesGuardados().forEach(quiz => {
          if (quiz.nombre.toLowerCase().includes(q) || quiz.materiaNombre?.toLowerCase().includes(q)) {
            encontrados.push({ tipo: 'pagina', titulo: quiz.nombre, subtitulo: `🤓 ${quiz.preguntas.length} ${idioma === 'en' ? 'questions' : 'preguntas'}${quiz.materiaNombre ? ` · ${quiz.materiaNombre}` : ''}`, emoji: '🤓', href: '/materias' });
          }
        });
        getFlashcardDecks().forEach(deck => {
          if (deck.nombre.toLowerCase().includes(q) || deck.materiaNombre?.toLowerCase().includes(q)) {
            encontrados.push({ tipo: 'pagina', titulo: deck.nombre, subtitulo: `🎴 ${deck.flashcards.length} flashcards${deck.materiaNombre ? ` · ${deck.materiaNombre}` : ''}`, emoji: '🎴', href: '/materias' });
          }
        });
      } catch {}
