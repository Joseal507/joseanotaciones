...
import { getNotas, saveNotas, crearNota, eliminarNota, actualizarNota, NotaRapida } from '../lib/notas';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        {notas.length > 3 && (
          <button onClick={() => setExpandido(!expandido)}
            ...
            {expandido ? 'Ver menos' : `Ver todas (${notas.length})`}
          </button>
        )}
      </div>
      ...
      {notas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px', background: 'var(--bg-card)', borderRadius: '16px', border: '2px dashed var(--border-color)' }}>
          ...
          <p style={{ color: 'var(--text-faint)', fontSize: '14px', margin: 0 }}>
            No tienes notas. Escribe una arriba.
          </p>
        </div>
      ) : (
