...
export default function EstudioModal({ flashcards, onClose, temaColor, onModoExamen, materiaId, materiaNombre, materiaColor }: Props) {
  ...
  if (modo === 'fin') {
    ...
    if (completadosUsados > 0) {
      ...
      if (materiaId) {
        const tipoSesion = esRepaso ? 'repaso' : 'estudio';
        const pctFlash = completadosUsados > 0 ? Math.round((acertadas2 / completadosUsados) * 100) : 0;
        import('../../lib/storage').then(({ registrarQuiz }) => {
          registrarQuiz(materiaId, materiaNombre || 'Estudio', materiaColor || temaColor, pctFlash, acertadas2, falladas2);
        }).catch(() => {});
      }
    }
