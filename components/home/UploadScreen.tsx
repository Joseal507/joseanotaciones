...
// ─── Tipos aceptados ───
const TIPOS_ACEPTADOS = ['.pdf', '.docx', '.txt', '.jpg', '.jpeg', '.png', '.webp', '.gif'];
const ACCEPT_STR = 'application/pdf,.pdf,.docx,.txt,image/*';
...
function detectarTipoArchivo(file: File): 'valido' | 'pptx' | 'otro' {
  ...
  // Aceptados
  if (
    nombre.endsWith('.pdf') || mime === 'application/pdf' ||
    nombre.endsWith('.docx') || mime.includes('wordprocessingml') ||
    nombre.endsWith('.txt') || mime === 'text/plain' ||
...
export default function UploadScreen({ file, loading, message, loadingStep, flashcardCount, onFileChange, onUpload, onSetCount, onVerDocumento }: Props) {
  ...
  // Wrapper que valida antes de pasar al handler real
  const handleFileChangeValidado = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (tipo === 'pptx' || tipo === 'otro') {
      ...
      // Limpiar el input para que pueda volver a elegir el mismo archivo
      e.target.value = '';
      return;
    }

    // Archivo válido → pasar al handler original
    onFileChange(e);
  };
      ...
      <div style={{ maxWidth: '580px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '28px' }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '13px', margin: 0 }}>PDF, Word, TXT o Imágenes</p>
          </div>
        </div>
          ...
          <div style={{ padding: '32px' }}>
            ...
            <label htmlFor="file-upload" style={{
              ...
              <input
                type="file"
                accept={ACCEPT_STR}
                onChange={handleFileChangeValidado}
                style={{ display: 'none' }}
                id="file-upload"
              />
