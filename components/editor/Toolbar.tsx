...
export default function Toolbar({
  ...
  const Btn = ({
    ...
    <button
      ...
      onMouseEnter={(e: any) => {
        if (!active) {
          e.currentTarget.style.background = temaColor;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          <span style={{ fontSize: '9px', color: 'var(--text-faint)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginRight: '2px' }}>
            Dibujar
          </span>
          <Btn
            label="🖊️ Bolígrafo"
            title="Bolígrafo"
            active={herramientaActiva === 'boligrafo'}
            onClick={() => onHerramienta('boligrafo')}
          />
          <Btn
            label="🖌️ Marcador"
            title="Marcador resaltador"
            active={herramientaActiva === 'marcador'}
            onClick={() => onHerramienta('marcador')}
          />
          <Btn
            label="✏️ Lápiz"
            title="Lápiz"
            active={herramientaActiva === 'lapiz'}
            onClick={() => onHerramienta('lapiz')}
          />
          <Btn
            label="🧹 Borrador"
            title="Borrador"
            active={herramientaActiva === 'borrador'}
            onClick={() => onHerramienta('borrador')}
          />
        </div>

        <Sep />

        {/* Colores - solo en modo dibujo */}
        {modoEscritura && (
          <>
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              <span style={{ fontSize: '9px', color: 'var(--text-faint)', fontWeight: 700, textTransform: 'uppercase', marginRight: '2px' }}>
                Color
              </span>
              {COLORES.map(c => (
                <button
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {[1, 2, 4, 7, 12, 20].map(s => (
                <button
                  ...
                  <div style={{
                    width: Math.min(s * 2.5, 26) + 'px',
                    height: Math.min(s * 2.5, 26) + 'px',
                    borderRadius: '50%',
                    background: herramientaActiva === 'borrador'
                      ? '#888'
                      : brushColor,
                    opacity: herramientaActiva === 'lapiz'
                      ? 0.75
                      : herramientaActiva === 'marcador'
                        ? 0.4
                        : 1,
                  }} />
                </button>
              ...
              />
              <span style={{ fontSize: '10px', color: 'var(--text-faint)', flexShrink: 0 }}>px</span>
            </div>
        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          ...
          <Btn
            label="🎨 Dibujo"
            title="Insertar dibujo en canvas"
            onClick={onInsertDibujo}
          />
          <Btn
            label="— Línea"
            title="Separador horizontal"
            onClick={() => onInsertHtml('<hr style="border:none;border-top:2px solid var(--border-color);margin:20px 0"/>')}
          />
          <Btn
            label="📌 Nota"
            title="Caja de nota"
            onClick={() => onInsertHtml(`<div style="background:${temaColor}18;border-left:4px solid ${temaColor};padding:12px 16px;border-radius:0 10px 10px 0;margin:12px 0;display:flex;gap:10px"><span>📌</span><div><strong style="color:${temaColor}">NOTA:</strong> escribe aquí...</div></div>`)}
          />
          <Btn
            label="⚠️ Aviso"
            title="Caja de aviso"
            onClick={() => onInsertHtml('<div style="background:#ff4d6d18;border-left:4px solid #ff4d6d;padding:12px 16px;border-radius:0 10px 10px 0;margin:12px 0;display:flex;gap:10px"><span>⚠️</span><div><strong style="color:#ff4d6d">IMPORTANTE:</strong> escribe aquí...</div></div>')}
          />
          <Btn
            label="💡 Tip"
            title="Caja de tip"
            onClick={() => onInsertHtml('<div style="background:#38bdf818;border-left:4px solid #38bdf8;padding:12px 16px;border-radius:0 10px 10px 0;margin:12px 0;display:flex;gap:10px"><span>💡</span><div><strong style="color:#38bdf8">TIP:</strong> escribe aquí...</div></div>')}
          />
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Alineación */}
        <Btn label="⬅" title="Alinear izquierda" onClick={() => onExecCmd('justifyLeft')} />
        <Btn label="⬛" title="Centrar" onClick={() => onExecCmd('justifyCenter')} />
        <Btn label="➡" title="Alinear derecha" onClick={() => onExecCmd('justifyRight')} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <input
            type="color"
            defaultValue="#ffffff"
            onChange={e => onExecCmd('foreColor', e.target.value)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <input
            type="color"
            defaultValue="#f5c842"
            onChange={e => onExecCmd('hiliteColor', e.target.value)}
