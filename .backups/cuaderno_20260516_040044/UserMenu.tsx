...
export default function UserMenu() {
  ...
  const navTo = (href: string) => { setAbierto(false); router.push(href); };

  const links = [
    { label: 'Mi Perfil',      emoji: '👤', href: '/perfil',    color: 'var(--gold)',  rot: -2 },
    { label: 'Mis Materias',   emoji: '📚', href: '/materias',  color: 'var(--blue)',  rot:  1.5 },
    { label: 'Configuración',  emoji: '⚙️', href: '/settings',  color: 'var(--pink)',  rot: -1.5 },
    { label: 'Comunidad',      emoji: '💬', href: '/comunidad', color: '#a855f7',      rot:  2 },
    { label: 'Partners',       emoji: '🤝', href: '/partners',  color: '#22c55e',      rot: -2 },
    { label: 'News',           emoji: '📰', href: '/news',      color: '#f97316',      rot:  1.5 },
  ];

  if (isAdmin) {
    links.push({ label: 'Admin Panel', emoji: '👑', href: '/news?admin=1', color: '#dc2626', rot: -1 });
  }

  return (
    <div ref={menuRef} style={{ position: 'relative' }}>
      {/* Tooltip */}
      {hoverTooltip && !abierto && (
        <div style={{
          ...
          <div style={{
            ...
            borderLeft: '8px solid var(--text-primary)',
          }}/>
        </div>
      )}
      ...
      {/* Dropdown — desciende como hoja de cuaderno */}
      {abierto && (
        <>
          {/* Overlay para detectar clicks afuera */}
          ...
          <div style={{
            ...
            animation: 'umSlide .35s cubic-bezier(.34,1.4,.64,1)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {links.map((item, i) => (
                <button key={i}
                  ...
                  onMouseEnter={(e: any) => {
                    e.currentTarget.style.background = `color-mix(in srgb,${item.color} 14%,transparent)`;
                    e.currentTarget.style.borderColor = item.color;
                    e.currentTarget.style.transform = `rotate(${item.rot}deg) translateX(4px)`;
                  }}
                  onMouseLeave={(e: any) => {
      ...
      <style>{`
        @keyframes umSlide {
          0% { opacity: 0; transform: translateY(-20px) rotate(0deg) scale(.95); }
