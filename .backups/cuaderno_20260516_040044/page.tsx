...
function WelcomeUser({ name }: { name: string }) {
  return (
    <div>
      <div style={{ fontFamily:HAND,fontSize:17,color:'var(--text-muted)',fontWeight:600,lineHeight:1,fontStyle:'italic',marginTop:-4 }}>welcome back</div>
      <div style={{ fontFamily:HAND,fontSize:32,fontWeight:900,color:'var(--text-primary)',lineHeight:1,marginTop:0 }}>{name||'estudiante'}</div>
...
export default function Home() {
  ...
  const navMateria = (materiaId?: string) => {
    if (materiaId) {
      ...
      // Pasar el ID por URL también para que la página lo detecte antes de renderizar
      const href = `/materias?open=${encodeURIComponent(materiaId)}`;
      if (navigating) return;
      setNavLabel('Materias');
      setNavColor('#f5c842');
      setNavEmoji('📚');
      setNavHref(href);
      setNavigating(true);
