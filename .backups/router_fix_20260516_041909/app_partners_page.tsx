      ...
      <div style={{ display: 'flex', height: isMobile ? 'calc(100vh - 60px)' : 'calc(100vh - 78px)', overflow: 'hidden' }}>
        <div style={{
          ...
          {vista === 'chats' && (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', borderBottom: '2px dashed var(--border-color)', flexShrink: 0 }}>
                <input value={busqChat} onChange={e => setBusqChat(e.target.value)} placeholder="🔍 buscar partner..."
                  style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {partners.length === 0 ? (
                ...
                <div key={p.id} style={{
                  ...
                  <Av user={p.partner} size={44} onClick={() => window.location.href = `/u/${p.partner.user_id}`} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{
                      fontFamily: HAND, fontSize: 18, fontWeight: 800,
                      color: 'var(--text-primary)',
                    ...
                    <>
                      ...
                      {solicitudes.map(s => (
                        <div key={s.id} style={{
                          ...
                          <Av user={s.partner} size={42} onClick={() => window.location.href = `/u/${s.partner.user_id}`} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{
                              fontFamily: HAND, fontSize: 17, fontWeight: 800,
          ...
          {vista === 'buscar' && (
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', borderBottom: '2px dashed var(--border-color)', flexShrink: 0 }}>
                <input autoFocus value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="🔍 buscar usuarios..."
                  style={{
                    width: '100%', padding: '8px 14px',
                    borderRadius: 10,
                    border: '2.5px solid var(--text-primary)',
                ...
                {listaUsuarios.map(u => {
                  ...
                  return (
                    <div key={u.user_id} style={{
                      ...
                      <Av user={u} size={42} onClick={() => window.location.href = `/u/${u.user_id}`} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{
                          fontFamily: HAND, fontSize: 17, fontWeight: 800,
                          color: 'var(--text-primary)',
                      ...
                      <button onClick={() => window.location.href = `/u/${u.user_id}`}
                        style={{
                          padding: '6px 8px', borderRadius: 8,
                          border: '1.5px dashed var(--text-faint)',
                          background: 'transparent', color: 'var(--text-muted)',
