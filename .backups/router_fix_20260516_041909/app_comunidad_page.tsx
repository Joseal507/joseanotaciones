...
function PostCard({ post, userId, onLike, onGuardar }: { post: Post; userId: string; onLike: (id: string) => void; onGuardar: (id: string) => void }) {
  ...
  const abrirPost = () => { window.location.href = `/comunidad/${post.id}`; };
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          <button onClick={e => { e.preventDefault(); e.stopPropagation(); onLike(post.id); }}
            style={{
          ...
          <button onClick={e => { e.preventDefault(); e.stopPropagation(); onGuardar(post.id); }}
            style={{
