            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <Field label={idioma === 'en' ? 'Start' : 'Inicio'}>
                  <select value={formInicio} onChange={e => setFormInicio(e.target.value)} style={inputStyle(formColor)}>
                    {HORAS.map(h => <option key={h} value={h}>{formatHora(h)}</option>)}
                  </select>
                </Field>
                <Field label={idioma === 'en' ? 'End' : 'Fin'}>
                  <select value={formFin} onChange={e => setFormFin(e.target.value)} style={inputStyle(formColor)}>
                    {HORAS.map(h => <option key={h} value={h}>{formatHora(h)}</option>)}
                  </select>
                </Field>
              </div>
