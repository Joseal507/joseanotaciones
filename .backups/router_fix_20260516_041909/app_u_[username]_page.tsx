        ...
        <div style={{ padding: '20px 28px', overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Field label={tr('nombre')}>
              <Input value={nombre} onChange={e => setNombre(e.target.value)} placeholder={tr('tuNombre')} />
            </Field>

            <Field label={`${tr('descripcion')} (${descripcion.length}/300)`}>
              <textarea value={descripcion} onChange={e => setDescripcion(e.target.value.slice(0, 300))}
              ...
              <>
                <Field label={`🏫 ${tr('universidad')}`}>
                  ...
                  {universidad === 'Otra universidad' && (
                    <div style={{ marginTop: 8 }}>
                      <Input value={uniCustom} onChange={e => setUniCustom(e.target.value)} placeholder={tr('escribeUniversidad')} />
                    </div>
                  )}
                </Field>
                <Field label={`📚 ${tr('carreraMajor')}`}>
                  ...
                  {carrera === 'Otra carrera' && (
                    <div style={{ marginTop: 8 }}>
                      <Input value={carreraCustom} onChange={e => setCarreraCustom(e.target.value)} placeholder={tr('escribeCarrera')} />
                    </div>
                  )}
                </Field>
              </>
            ...
            {tipo === 'escuela' && (
              <Field label={`🏫 ${tr('escuela')}`}>
                ...
                {escuela === 'Otra escuela' && (
                  <div style={{ marginTop: 8 }}>
                    <Input value={escuelaCustom} onChange={e => setEscuelaCustom(e.target.value)} placeholder={tr('escribeEscuela')} />
                  </div>
                )}
              </Field>
            )}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {esMiPerfil && (
            <>
              ...
              <HeaderBtn onClick={() => (window.location.href = '/perfil')} color="var(--blue)" rot={1}>
                📊 {tr('misStatsBtn')}
              </HeaderBtn>
            </>
          )}
          ...
          {!esMiPerfil && miUserId && (
            <>
              ...
              {partnerStatus === 'partner' && (
                <HeaderBtn onClick={() => window.location.href = '/partners'} color="#4ade80" rot={1}>
                  👥 {tr('partners')}
                </HeaderBtn>
              )}
