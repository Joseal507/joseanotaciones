          ...
          return (
            ...
            >
              {esVideo ? (
                <>
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', zIndex: 2 }}>
                    <div style={{ flex: 1 }} onClick={e => { e.stopPropagation(); handleZoneTap(post, 'left'); }} />
                    <div style={{ flex: 1 }} onClick={e => { e.stopPropagation(); handleZoneTap(post, 'center'); }} />
                    <div style={{ flex: 1 }} onClick={e => { e.stopPropagation(); handleZoneTap(post, 'right'); }} />
                  </div>
                </>
              ) : post.portada_url ? (
                <img
              ...
              <div style={{
                ...
                {/* Like */}
                <button onClick={e => { e.stopPropagation(); void triggerLike(post); }}
                ...
                {/* Guardar */}
                <button onClick={e => { e.stopPropagation(); void toggleGuardar(post); }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer' }}
                >
                  <div style={{
                    width: 54, height: 54, borderRadius: '50%',
              ...
              {/* Mute video */}
              {esVideo && (
                <button onClick={e => { e.stopPropagation(); void toggleMute(post.id); }}
                  style={{
                    position: 'absolute',
                    top: 68, right: 14,
                    zIndex: 3,
              ...
              <div style={{
                ...
                {posts.slice(Math.max(0, current - 3), current + 5).map((p, i) => {
                  ...
                  return (
                    <div key={p.id}
                      onClick={e => { e.stopPropagation(); scrollToIndex(realIndex); }}
                      style={{
                        width: active ? 5 : 4,
                        height: active ? 28 : 9,
                        borderRadius: 999,
