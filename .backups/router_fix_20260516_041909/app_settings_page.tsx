          ...
          {seccion === 'perfil' && (
            <NotebookCard color="var(--gold)" emoji="👤" title={tr('publicProfile')} rot={-0.4}>
              ...
              <Field label={tr('nombre')}>
                <Input value={nombre} onChange={e => setNombre(e.target.value)} placeholder={tr('nombre')} />
              </Field>
                ...
                <>
                  <Field label={`🏫 ${tr('universidad').replace('🏫 ', '')}`}>
                    ...
                    {universidad === 'Otra universidad' && (
                      <Input value={uniCustom} onChange={e => setUniCustom(e.target.value)} placeholder={tr('otraUniversidad')} style={{ marginTop: 8 }} />
                    )}
                  </Field>
                  <Field label={`📚 ${tr('carreraMajor').replace('📚 ', '')}`}>
                    ...
                    {carrera === 'Otra carrera' && (
                      <Input value={carreraCustom} onChange={e => setCarreraCustom(e.target.value)} placeholder={tr('otraCarrera')} style={{ marginTop: 8 }} />
                    )}
                  </Field>
                </>
              )}
              ...
              {tipoEstudiante === 'escuela' && (
                <Field label={`🏫 ${idioma === 'en' ? 'School' : 'Escuela'}`}>
                  ...
                  {escuela === 'Otra escuela' && (
                    <Input value={escuelaCustom} onChange={e => setEscuelaCustom(e.target.value)} placeholder={tr('otraEscuela')} style={{ marginTop: 8 }} />
                  )}
                </Field>
              )}
              ...
              <NotebookCard color="var(--pink)" emoji="🎨" title={tr('temaColores')} rot={0.4}>
                ...
                <div style={{
                  ...
                  <Field label={tr('nombreTema')}>
                    <Input value={customName} onChange={e => setCustomName(e.target.value.slice(0, 20))} placeholder={tr('miTema')} />
                  </Field>
          ...
          {seccion === 'notificaciones' && (
            <>
              ...
              <NotebookCard color="var(--blue)" emoji="🔔" title={tr('notificaciones')} rot={0.4}>
                ...
                <p style={{ fontFamily: HAND, fontSize: 14, fontStyle: 'italic', color: 'var(--text-faint)', margin: 0 }}>
                  ~ {tr('soloNavegador')} ~
                </p>
              </NotebookCard>
            </>
